...
from utils.validate_fields import check_account_fields
...
def test_good_user(mongodb):
    status = check_account_fields(generate_good_username(), generate_good_email(), good_pass)[0]
    assert status == 0, "Failed test for all valid fields"

def test_bad_user():
    username = generate_bad_username()
    status = check_account_fields(username, generate_good_email(), good_pass)[0]
    assert status == 1 or status == 2, "Failed test for bad username"

def test_empty_user():
    status = check_account_fields("", generate_good_email(), good_pass)[0]
    assert status == 1, "Failed test for empty username"

def test_empty_pass():
    status = check_account_fields(generate_good_username(), generate_good_email(), "")[0]
    assert status == 4, "Failed test for empty password"

def test_bad_email():
    status = check_account_fields(generate_good_username(), bad_email, good_pass)[0]
...
def test_existing_email(mongodb):
    try:
        ...
        user.push()
        status = check_account_fields(generate_good_username(), user.email, good_pass)[0]
...
def test_existing_user(mongodb):
    try:
        user = generate_user(True)
        user.push()
        status = check_account_fields(user.username, generate_good_email(), good_pass)[0]
