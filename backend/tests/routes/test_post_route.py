...
def test_dislike_post(test_client):
    try:
        # simulate logged in user
        user = generate_user(True)
        response = test_client.post("/api/user/createaccount", json={
        ...
        # track initial number of dislikes on this post
        dislikeCount = len(post.dislikes)
        # user dislikes this post
        response = test_client.post("/api/post/dislikepost", json={
            "post_id": post.post_id
            })
        data = response.json
        assert response.status_code == 200, "Bad dislike post response, got " + str(response.status_code)
        post = Post.find(post.post_id)
        user = User.find_by_email(user.email)
        assert data["success"] == True and len(post.dislikes) == dislikeCount + 1 and userId in post.dislikes and post.post_id in user.disliked_posts, f"Dislike post test failed for post {post.post_id}, got success {data.get('success', None)}"
    finally:
        if User.find_by_email(user.email):
            User.delete_by_email(user.email)
        if Post.find(post.post_id):
...
def test_undislike_post(test_client):
    try:
        # simulate logged in user
        user = generate_user(True)
        response = test_client.post("/api/user/createaccount", json={
        ...
        # track initial number of dislikes
        dislikeCount = len(post.dislikes)
        # user removes their dislike
        response = test_client.post("/api/post/undislikepost", json={
            "post_id": post.post_id
        })
        data = response.json
        assert response.status_code == 200, "Bad undislike post response, got " + str(response.status_code)
        post = Post.find(post.post_id)
        user = User.find_by_email(user.email)
        assert data["success"] == True and len(post.dislikes) == dislikeCount - 1 and userId not in post.dislikes and post.post_id not in user.disliked_posts, f"Undislike post test failed for post {post.post_id}, got success {data.get('success', None)}"
    finally:
        if User.find_by_email(user.email):
            User.delete_by_email(user.email)
        if Post.find(post.post_id):
