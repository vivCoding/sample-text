...
def test_follow_topic(mongodb):
    if User.find_by_email(good_user.email) is None:
        good_user.push()
    assert good_user.follow_topic(good_topic.topic_name) and good_topic.topic_name in User.find_by_email(good_user.email).followed_topics, "Failed to follow topic"
...
def test_unfollow_topic(mongodb):
    assert good_user.unfollow_topic(good_topic.topic_name) and good_topic.topic_name not in User.find_by_email(good_user.email).followed_topics, "Failed to unfollow topic"
